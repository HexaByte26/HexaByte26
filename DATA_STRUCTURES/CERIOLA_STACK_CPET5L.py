class DiskStorageStack:
    ...
    def pop_disk(self):
        if self.is_empty():
            print("Log: Cannot pop from an empty Disk Storage")

        self.disks[self.top_disk_index] = self.place_holder
        self.top_disk_index -= 1

        print(f"Log: Disk {self.disks[self.top_disk_index]} has been popped from the Disk Storage")

    def peek_top_disk(self):
        print(f"Log: {self.disks[self.top_disk_index - 1]} is the top most disk")

    def get_disks_num(self):
        print(f"Log: Currently there are {self.top_disk_index} disks inside of the Disk Storage")
